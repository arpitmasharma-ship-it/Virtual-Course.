...
const SignUp = () => {
  ...
  const [show, setShow] = useState(false)  /* PASSWORD SHOW OR NOT SHOW USE KRNA KE LIYA  */

  const navigate = useNavigate()



  /* axios kya kreag jo rouets or conmtrollers hamena banaya hai unko fetch kraga . */
  ...
  /* jo bhi upper 4 states likha hai inki value bhi set krni hai niche by doing onchange */



  /* jb jb ham signin krwaye tb tb hamra  setCurrentUser ka jo data hai vo fir se set ho jaye */
  ...
  const handelSignup = async (e) => {
    ...
    try {
      ...
      toast.done("Signup Succewssfully")
      navigate("/")
  ...
  return (
    ...
    <div className="bg-[#dadbab] w-[100vw] h-[100vh] flex items-center justify-center">
      <form onSubmit={handelSignup} className="w-[90%] md:w-200 h-150 bg-white shadow-xl rounded-2xl flex ">
        ...
        <div className="md:w-[50%] w-[100%] h-[100%] flex flex-col items-center justify-center gap-3">
          ...
          <div className='flex flex-col gap-1  w-[80%] items-start justify-center px-3' >
            ...
            {/* 
            e is the event object. React gives you information about what happened.
            e.target   => For an input, you can access the input element through .,......
            e.target.value   =>    "Give me the current text inside the input."
            */}

            <label htmlFor="name" className='font-semibold'  >Name</label>   {/* yeha htmlFor = "name"   mtlb ki jb ma label pa bhi click kruga tho mera input pe click ho jeyga  */}
            <input onChange={(e) => setName(e.target.value)} value={name} id='name' type="text" placeholder="Enter Your Name" className='border-1 w-[100%] h-[35px] border-[#e7e6e6] text-[15px] px-[20px]    ' />

            {/* In Initial useState  name = ""    onchange =>"Whenever the user changes what is inside the input, run this function." */}
            {/* e.target.value    jo bhi user ne input ma type kiya vo  */}
            {/* VALUE KA MATLB HAI KI KYA SHOW HOGA USER KO INPUT MA  */}
          ...
          <div className='flex  md:w-[50%]  w-[70%]  items-center  justify-between   '      >
            <span
              ...
              Student  </span>
            <span className={`" hover:shadow-md px-[10px] py-[5px] border-[1px] border-[#e7e6e6] rounded-xl cursor-pointer hover:border-black"
          ...
          {/* Button for Sign */}  {/* to sumbit  the Form  */}
          <button
            className="w-[80%] h-[40px] bg-black text-white cursor-pointer flex items-center justify-center rounded-[5px]" disabled={loading} onChange={handelSignup} >
            {/* disabled={loading}   mtlb ager loading ki value true hui tho button disable ho  jayega .... */}
            {loading ? <ClipLoader size={30} color='white' /> : "SignUp"}  {/* my miastake is ki mean "SignUp"     is ki jagha only Signup likh  diya tha . */}
            {       /* loading true ka m,tlb hai ki hb hamera paas data nhi aaya hai .  */}
          </button>
          {/*          Or Sapce   */}
          ...
          {/* Google Login */}
          <div onClick={googleSignUp} className=' cursor-pointer w-[80%] h-[40px] border-1 border-[black] rounded-[5px] flex items-center justify-center'>
