...
function CardPage() {
    ...
    /* Yeha mujhe IsPublished wala courses leke aana hai ... */
    const { courseData } = useSelector(state => state.course)
    const [popularCourses, setPopularCourses] = useState([]);


    /* UseEffect ka kaam ye hai ki jasa hi mera commponent Render ho yho sidha mera ye useEffect chalana laga jaye ... */
    useEffect(() => {
...
index  = current course ki position */}
                {/* <Card key={index} />
Matlab:
React → ye first Card hai
        ye second Card hai
        ye third Card hai */}
                {
                    popularCourses.map((course, index) => (  /* My Mistake  is ki mena yeah es ( ) ki jagha ye { } ye laga diya  ...  */
